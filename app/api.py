...
app = FastAPI(
    ...
    description="API REST pour la gestion d'une bibliotheque de livres avec stockage PostgreSQL",
    version="2.0.0",
    contact={"name": "Equipe Bibliotheque", "email": "ahmadSchema@.coding"},
)
...
def racine():
    return {
        ...
        "endpoints": {
            "livres": "/livres",
            "livres_disponibles": "/livres/disponibles",
...
def rechercher(q: str):
    if len(q.strip()) < 2:
        raise HTTPException(status_code=400, detail="Le terme de recherche doit contenir au moins 2 caracteres.")
    try:
...
@app.post(
    ...
    responses={
        400: {"model": ErrorResponse, "description": "Donnees invalides ou ISBN existe deja"}
    },
)
def creer_livre(livre: LivreCreate):
    try:
        return service.ajouter_livre(
            livre.isbn, 
            livre.titre, 
            livre.auteur, 
            livre.disponible if hasattr(livre, 'disponible') else True
        )
    except ServiceError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def modifier_livre(isbn: str, maj: LivreUpdate):
    try:
        return service.modifier(
            isbn, 
            maj.titre if hasattr(maj, 'titre') else None, 
            maj.auteur if hasattr(maj, 'auteur') else None, 
            maj.disponible if hasattr(maj, 'disponible') else None
        )
    except ServiceError as e:
